...
import { getCurrentUserActiveAppointments, getCurrentUserAppointmentsHistory } from '../services/appointmentService'

const AppointmentsHistoryScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const freeH = async () => {
      setBusy(true);
      var appointments = await getCurrentUserAppointmentsHistory();
  ...
  return (
    <View>
      <Header
          ...
          centerComponent={{ text: t('lang:history'), style: { color: '#fff' } }}
          rightComponent={
            <TouchableOpacity
